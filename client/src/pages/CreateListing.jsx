...
const CreateListing = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (formData.imageUrls.length < 1) return setError('You must add atleast one image')
      if (+formData.regularPrice < +formData.discountPrice) return setError('Discount Price mUst be Lower than the regular price') //+ isliye coz string ko number me typecast kiya 
      setLoading(true);
      setError(false);
      ...
      <form className="flex flex-col sm:flex-row gap-4" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-4 flex-1">
          ...
          <div className="flex flex-wrap gap-6 ">
            ...
            <div className=" flex items-center gap-2">
              <input type="number" id='regularPrice' min='500' max='20000' required className='p-3 border border-gray-300 rounded-lg'
        ...
        <div className="flex flex-col flex-1 gap-4">
          <p className='font-semibold '>Images :
            <span className="font-normal text-gray-700 ml-2">The First image will be cover (max 6)</span></p>
          ...
          <button className="p-3 bg-slate-700 text-white rounded-lg uppercase hover:opacity-95 disabled:opacity-80"
            disabled={loading || uploading}> {loading ? "Loading" : "Create Listing "}
          </button>
